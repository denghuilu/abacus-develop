...
namespace ModuleESolver {
    ...
    void ESolver_KS::init_after_vc(Input &inp, UnitCell &ucell) {
    ...
    void ESolver_KS::print_wfcfft(Input &inp, std::ofstream &ofs) {
        ofs << "\n\n\n\n";
        ofs << " >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" << std::endl;
        ofs << " |                                                                    |" << std::endl;
        ...
        ModuleBase::GlobalFunc::OUT(ofs, "fft grid for wave functions", this->pw_wfc->nx, this->pw_wfc->ny,
                                    this->pw_wfc->nz);
                ...
                if (GlobalV::MY_STOGROUP == 0) {
                    ...
                    if (drho < hsolver_error || this->conv_elec) {
                        if (drho < hsolver_error)
                            GlobalV::ofs_warning << " drho < hsolver_error, keep charge density unchanged."
                                                 << std::endl;
                    } else {
                        //----------charge mixing---------------
    ...
    ModuleIO::Output_Rho ESolver_KS::create_Output_Rho(int is, int iter, const std::string &prefix) {
        int precision = 3;
        std::string tag = "CHG";
    ...
    ModuleIO::Output_Rho ESolver_KS::create_Output_Kin(int is, int iter, const std::string &prefix) {
        int precision = 11;
        std::string tag = "TAU";
        return ModuleIO::Output_Rho(this->pw_big,
                                    this->pw_rho,
    ...
    ModuleIO::Output_Potential ESolver_KS::create_Output_Potential(int iter, const std::string &prefix) {
        int precision = 3;
        std::string tag = "POT";
