...
template <typename FPTYPE, typename Device> DiagoCG<FPTYPE, Device>::DiagoCG(const FPTYPE *precondition_in)
{
    ...
    // #ifdef __CUDA
    //     hsolver::createCUBLAShandle();
    // #endif
}

template <typename FPTYPE, typename Device> DiagoCG<FPTYPE, Device>::~DiagoCG()
{
    ...
    // #ifdef __CUDA
    //     hsolver::destroyCUBLAShandle();
    // #endif
}

template <typename FPTYPE, typename Device>
void DiagoCG<FPTYPE, Device>::diag_mock(hamilt::Hamilt *phm_in,
...
template <>
void DiagoCG<double, psi::DEVICE_GPU>::diag_mock(hamilt::Hamilt *phm_in,
                                                 psi::Psi<std::complex<double>, psi::DEVICE_GPU> &phi,
                                                 double *eigenvalue_in)
{
    ...
    for (int m = 0; m < this->n_band; m++)
    {
        ...
        this->eigenvalue[m]
            = hsolver::zdot_real(this->dim, this->phi_m->get_pointer(), this->hphi, this->phi_m->get_device());
...
template <typename FPTYPE, typename Device> void DiagoCG<FPTYPE, Device>::calculate_gradient()
{
    if (this->test_cg == 1)
    ...
    //(2) PH|psi>
    hsolver::vector_div_vector(this->dim, this->gradient, this->hphi, this->precondition);
    //(3) PS|psi>
    hsolver::vector_div_vector(this->dim, this->pphi, this->sphi, this->precondition);
    ...
    // }
    // haozhihan replace this 2022-10-6
    hsolver::vector_sub_constantVector(this->dim, this->gradient, this->gradient, this->pphi, lambda);
...
template <typename FPTYPE, typename Device>
void DiagoCG<FPTYPE, Device>::orthogonal_gradient(hamilt::Hamilt *phm_in,
                                                  const psi::Psi<std::complex<FPTYPE>> &eigenfunction,
                                                  const int m)
{
    if (test_cg == 1)
...
template <typename FPTYPE, typename Device>
void DiagoCG<FPTYPE, Device>::calculate_gamma_cg(const int iter,
                                                 FPTYPE &gg_last,
                                                 const FPTYPE &cg_norm,
                                                 const FPTYPE &theta)
{
    if (test_cg == 1)
    ...
    if (iter > 0)
    {
        ...
        gg_inter = hsolver::zdot_real(this->dim, this->gradient, this->g0); // b means before
    }
    ...
    // }
    // haozhihan replace this 2022-10-6
    hsolver::vector_mul_vector(this->dim, this->g0, this->scg, this->precondition);
    ...
    else
    {
        ...
        // }
        // haozhihan replace this 2022-10-6
        hsolver::vector_add_constantVector(this->dim, pcg, this->gradient, pcg, gamma);
...
bool DiagoCG<FPTYPE, Device>::update_psi(FPTYPE &cg_norm, FPTYPE &theta, FPTYPE &eigenvalue)
{
    ...
    const FPTYPE a0 = hsolver::zdot_real(this->dim, phi_m_pointer, this->pphi) * 2.0 / cg_norm;
    const FPTYPE b0 = hsolver::zdot_real(this->dim, this->cg->get_pointer(), this->pphi) / (cg_norm * cg_norm);
    ...
    // }
    // haozhihan replace this 2022-10-6
    hsolver::constantvector_add_constantVector(this->dim, phi_m_pointer, phi_m_pointer, cost, pcg, sint_norm);
    ...
    else
    {
        ...
        // }
        // haozhihan replace this 2022-10-6
        hsolver::constantvector_add_constantVector(this->dim, this->sphi, this->sphi, cost, this->scg, sint_norm);
        hsolver::constantvector_add_constantVector(this->dim, this->hphi, this->hphi, cost, this->pphi, sint_norm);
...
template <typename FPTYPE, typename Device>
void DiagoCG<FPTYPE, Device>::schmit_orth(const int &m, // end
                                          const psi::Psi<std::complex<FPTYPE>, Device> &psi)
{
    ...
    //     pphi_m[ig] /= psi_norm;
    // }
    hsolver::vector_div_constant(this->dim, pphi_m, pphi_m, psi_norm);
...
template <typename FPTYPE, typename Device>
void DiagoCG<FPTYPE, Device>::diag(hamilt::Hamilt *phm_in,
