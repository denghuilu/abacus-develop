...
void DiagoDavid<T, Device>::diag_mock(hamilt::Hamilt<R, Device>* phm_in,
                                           ...
                                           R* eigenvalue_in)
{
    ...
    psi::memory::resize_memory_op<R, psi::DEVICE_CPU>()(this->cpu_ctx, this->eigenvalue, this->nbase_x, "DAV::eig");
    psi::memory::set_memory_op<R, psi::DEVICE_CPU>()(this->cpu_ctx, this->eigenvalue, 0, this->nbase_x);
...
template <typename T, typename Device>
void DiagoDavid<T, Device>::cal_grad(hamilt::Hamilt<R, Device>* phm_in,
                                          const int& dim,
                                          ...
                                          const R* eigenvalue)
{
    ...
    for (int m = 0; m < notconv; m++)
    {
        ...
        {
#if defined(__CUDA) || defined(__ROCM)
            vector_div_vector_op<R, Device>()(this->ctx,
                                                   this->dim,
                                                   &basis(nbase + m, 0),
                                                   &basis(nbase + m, 0),
                                                   this->d_precondition);
                                          ...
                                          T* scc)
{
    ...
    {
        matrixTranspose_op<R, Device>()(this->ctx, this->nbase_x, this->nbase_x, hcc, hcc);
        matrixTranspose_op<R, Device>()(this->ctx, this->nbase_x, this->nbase_x, scc, scc);
                                             ...
                                             T* vcc)
{
    ...
    if (GlobalV::RANK_IN_POOL == 0)
    {
        ...
        if (this->device == psi::GpuDevice)
        {
            ...
            dnevx_op<R, Device>()(this->ctx, nbase, this->nbase_x, this->hcc, nband, eigenvalue_gpu, this->vcc);
        ...
        {
            dnevx_op<R, Device>()(this->ctx, nbase, this->nbase_x, this->hcc, nband, this->eigenvalue, this->vcc);
        }
    }
                                            ...
                                            const int mv_size)
{
    ...
    psi_norm -= zdot_real_op<R, Device>()(this->ctx, m, lagrange_m, lagrange_m, false);
    ...
    else
    {
        //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        // haozhihan repalce 2022-10-16
        vector_div_constant_op<R, Device>()(this->ctx, this->dim, psi_m, psi_m, psi_norm);
        //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        // for (int i = 0; i < npw; i++)
        // {
        //     psi_m[i] /= psi_norm;
...
template <typename T, typename Device>
void DiagoDavid<T, Device>::diag(hamilt::Hamilt<R, Device>* phm_in,
                                      psi::Psi<T, Device>& psi,
                                      R* eigenvalue_in)
{
