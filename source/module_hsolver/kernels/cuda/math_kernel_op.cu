        ...
        const int dim)
{
    ...
    for (int ii = tid; ii < bid + 1; ii += THREAD_PER_BLOCK) {
        if (uplo == 'L') {
            A[ii * dim + bid + 1] = thrust::complex<FPTYPE>(0, 0);
        }
        else {
            A[(bid + 1) * dim + ii] = thrust::complex<FPTYPE>(0, 0);
        }
    }
}

template <typename FPTYPE>
__global__ void mat_add_inplace(
        thrust::complex<FPTYPE>* A,
        thrust::complex<FPTYPE>* B,
        const int n_basis,
        const int n_basis_max)
{
    int bid = blockIdx.x;
    int tid = threadIdx.x;
    for (int ii = tid; ii < n_basis; ii += THREAD_PER_BLOCK) {
        A[bid * n_basis_max + ii] += B[bid * n_basis_max + ii];
    }
}
...
template <typename FPTYPE>
void set_matrix_op<FPTYPE, psi::DEVICE_GPU>::operator()(
        const psi::DEVICE_GPU* /* dev */,
        const char &uplo,
        std::complex<FPTYPE>* A,
        const int &dim)
{
    auto thrust_A = reinterpret_cast<thrust::complex<FPTYPE>*>(A);
    set_matrix<FPTYPE><<<dim - 1, THREAD_PER_BLOCK>>>(uplo, thrust_A, dim);
}
...
template <typename FPTYPE>
void mat_add_inplace_op<FPTYPE, psi::DEVICE_GPU>::operator()(
        const psi::DEVICE_GPU* /* dev */,
        std::complex<FPTYPE>* A,
...
{
    auto thrust_A = reinterpret_cast<thrust::complex<FPTYPE>*>(A);
    auto thrust_B = reinterpret_cast<thrust::complex<FPTYPE>*>(B);
    mat_add_inplace<FPTYPE><<<n_band, THREAD_PER_BLOCK>>>(thrust_A, thrust_B, n_basis, n_basis_max);
}
