...
struct set_matrix_op<FPTYPE, psi::DEVICE_CPU> {
    void operator() (
        ...
        const int &dim)
    {
        for (int ii = 1; ii < dim; ii++) {
            for (int jj = 0; jj < ii; jj++) {
                if (uplo == 'L') {
                    A[jj * dim + ii] = std::complex<FPTYPE>(0, 0);
                }
                else {
                    A[ii * dim + jj] = std::complex<FPTYPE>(0, 0);
                }
            }
        }
    }
...
template <typename FPTYPE>
struct mat_add_inplace_op<FPTYPE, psi::DEVICE_CPU> {
    void operator() (
        const psi::DEVICE_CPU* /* dev */,
        std::complex<FPTYPE>* A,
        ...
        const int &n_basis_max)
    {
        for (int ii = 0; ii < n_band; ii++) {
            for (int jj = 0; jj < n_basis; jj++) {
                A[ii * n_basis_max + jj] += B[ii * n_basis_max + jj];
            }
        }
    }
};
